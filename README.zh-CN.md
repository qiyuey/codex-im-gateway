...
回复路由是持久化的：网关不会把回复悄悄转发到无关的当前任务。如果只有宿主继承的可信
任务 ID，通知会直接提供“切换到此任务”；没有可信任务身份或身份冲突时，才会标为
“独立通知”并提供任务选择器，而不会猜测回复目标。
...
Codex 没有为本次投递提供可信的任务身份，或宿主身份彼此冲突。网关仍会发送结果，但不会
猜测后续回复应该进入哪个任务。仅缺少执行元数据时，宿主继承的可信任务 ID 仍会提供直接
切换按钮。
